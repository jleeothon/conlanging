...
const wordListTemplate = readFileSync(wordListPath, 'utf-8');
const readmeTemplate = readFileSync(readmePath, 'utf-8');
...
function run() {
	const allEntries = yaml.load(readFileSync(0, 'utf-8'));
	...
	// Parts
	const partMapping = parts.map(p => ({name: p, fileName: `${p}.md`, link: `${p}.md`}));
	...
	// Templates
	const templateMapping = templates.map(t => ({name: t, fileName: `${t}.md`, link: `${t}.md`}));
	...
	// Tags
	const tagMapping = tags.map(c => ({name: c, fileName: `${c}.md`, link: `${c}.md`}));
